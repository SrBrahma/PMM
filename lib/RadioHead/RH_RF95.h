...
#define RH_RF95_PA_RAMP_250US                       0x04    // Fixed by Henrique Bruno. Was 0x0, and the register map shows is 0x04.
...
// General / Not so usual functions
...
// Reception
    ...
    /// If a message is copied, The length, the SNR and the RSSI are copied to the given receivedPacketPhysicalLayerInfoStruct.
    /// You should be sure to call this function frequently enough to not miss any messages
    /// \return true if a valid message was copied to buf
    bool    receivePayloadAndInfoStruct(uint8_t payload[], receivedPacketPhysicalLayerInfoStructType* receivedPacketPhysicalLayerInfoStruct);
    ...
    /// You should be sure to call this function frequently enough to not miss any messages
    /// \return true if a valid message was copied to buf
    bool    receivePayload(uint8_t* packet, uint8_t* packetLength);
...
// Getters!
    ...
    bool    getIsThereANewReceivedPacket();
...
private:
    ...
    uint8_t mReceivedPacketLength;                // Number of octets in the buffer

    bool    mIsThereANewReceivedPacket;                 // True when there is a valid message in the buffer

    int8_t  mLastSNR;                                   // Last measured SNR, dB
    int8_t  mLastRssi;                                  // The value of the last received RSSI value, in some transport specific units
