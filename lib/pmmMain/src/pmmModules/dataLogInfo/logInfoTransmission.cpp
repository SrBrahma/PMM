...
// 1) Adds the DataLogInfo Header to the packet
    ...
    mPacketStruct.payload[PORT_LOG_INFO_INDEX_DATA_LOG_ID]    = mDataLogId;
    // 1.6) Add the DataLog Group Length
    mPacketStruct.payload[PORT_LOG_INFO_INDEX_DATA_LOG_GROUP_LENGTH] = mGroupLength;
    
    mPacketStruct.payloadLength = PORT_LOG_INFO_HEADER_LENGTH;
...
// 2) Adds the DataLogInfo Payload, which was built on updateLogInfoCombinedPayload().
    ...
    // 2.2) Add the Payload.
    memcpy(mPacketStruct.payload + mPacketStruct.payloadLength, mLogInfoContentArray + (requestedPacket * PORT_LOG_INFO_MAX_PAYLOAD_LENGTH), payloadBytesInThisPacket);
